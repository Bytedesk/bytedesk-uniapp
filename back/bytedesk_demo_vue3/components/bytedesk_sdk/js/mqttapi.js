...
export function mqttConnect (callback) {
	...
	const username = uni.getStorageSync(constants.VISITOR_UID);
	let deviceUid = uni.getStorageSync(constants.VISITOR_DEVICEUID);
	if (deviceUid === '' || deviceUid === null || deviceUid === undefined) {
		deviceUid = utils.guid();
		uni.setStorageSync(constants.VISITOR_DEVICEUID, deviceUid);
	}
	// let clientId = uid + '/' + constants.client
	let clientId = uid + "/" + constants.HTTP_CLIENT + "/" + deviceUid;
	if (!constants.IS_DEBUG) {
		...
		const options = {
			...
			rejectUnauthorized: false,
			// log(...args) {
			//   console.log("mqtt dev log", ...args);
			// },
		  };
		  console.log("mqtt start debug connect:", options);
		  // mqttClient = mqtt.connect(options);
		  mqttClient = mqtt.connect("ws://127.0.0.1:9885/websocket", options);
	}      
