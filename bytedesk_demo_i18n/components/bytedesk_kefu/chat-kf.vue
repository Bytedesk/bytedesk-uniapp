			...
			<scroll-view class="msg-list" scroll-y="true" :scroll-with-animation="scrollAnimation" :scroll-top="scrollTop" :scroll-into-view="scrollToView" @scrolltoupper="loadMoreMessages" upper-threshold="50">
				...
				<view class="row" v-for="(message, index) in messages" :key="index" :id="'msg'+message.mid">
					...
					<view class="system">
						...
						<view class="text" v-else-if="is_type_notification_queue_accept(message)">{{ $t('acceptqueuethread') }}</view>
							...
							<view class="right">
								...
								<view v-if="is_type_robot_result(message)" class="bubble">
									<view class="flex-column-start" style="color: #2fa39b;">
										...
										<hr class="hr-solid">
										<view class="flex-row-start padding-top-sm">
											<text style="color: #007AFF; font-size: 5px;" @click="rateAnswerHelpful(message.answer.aid, message.mid)">{{ $t('helpful') }}</text>
											<text class="padding-left" style="color: #007AFF; margin-left: 10px; font-size: 5px;" @click="rateAnswerHelpless(message.answer.aid, message.mid)">{{ $t('helpless') }}</text>
										</view>
									</view>
								</view>
							</view>
