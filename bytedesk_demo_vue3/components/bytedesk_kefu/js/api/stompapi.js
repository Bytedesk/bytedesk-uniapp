...
import { Stomp } from './stompjs' // './stomp.min.js'
...
export function connect (thread, callback) {
    ...
    function connectWebSocket() {
	  ...
	  socketTask = uni.connectSocket({
	  	...
	  	success(res) {
			...
			printLog("socketTask success")
	  	},
	  	fail(err) {
	  		// 这里是接口调用失败的回调，不是连接失败的回调，请注意
			printLog("socketTask failed")
	  	}
	  })
	  // console.log(socketTask);
	  
	  socketTask.onOpen((result) => {
	  	printLog("SocketOpen:" + result)
    ...
    //
    connectWebSocket();
	stompConnect(webSocket, callback)
}
   ...
   * TODO: 加载所属工作组，添加订阅
   */
export function stompConnect (webSocket, callback) {
    /**
     * 定期发送心跳或检测服务器心跳
