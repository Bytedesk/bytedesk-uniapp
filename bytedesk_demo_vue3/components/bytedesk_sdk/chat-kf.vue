							...
							<view class="right">
								...
								<!-- <view v-if="is_type_robot_result(message)" class="bubble">
									<view class="flex-column-start" style="color: #2fa39b;">
										...
										<hr class="hr-solid">
										<view class="flex-row-start padding-top-sm">
											<text style="color: #007AFF; font-size: 5px;" @click="rateAnswerHelpful(message.answer.aid, message.uid)">有帮助</text>
											<text class="padding-left" style="color: #007AFF; margin-left: 10px; font-size: 5px;" @click="rateAnswerHelpless(message.answer.aid, message.uid)">没帮助</text>
										</view>
									</view>
								</view> -->
							</view>
...
// import * as mqttApi from '@/components/bytedesk_sdk/js/mqttapi.js'
...
export default {
	...
	methods:{
		...
		// 请求会话
		requestThread () {
			// console.log('request thread:', this.option.type, this.option.sid);
			let app = this
		...
		sendMessageJsonRest (uid, type, content) {
			console.log('sendMessageJsonRest:', uid, type, content)
		},
		// 实际发送消息
		doSendMessage (json) {
		...
		pushToMessageArray(message) {
			...
			for (var i = 0; i < this.messages.length; i++) {
				...
				if (msg.uid === message.uid) {
					...
					this.messages[i].status = message.status
				}
			}
			// 如果不存在，则保存
			if (!contains) {
				console.log('pushToMessageArray: push', message)
				...
				console.log('pushToMessageArray: update message status')
			}
		...
		byteDeskConnect () {
			...
			if (stompApi.isConnected()) {
				// 订阅topic
				let topic = this.thread.topic.replace(/\//g, ".")
				stompApi.subscribeTopic(topic, orgUid);
				// 获取本地缓存聊天记录
				...
				let topic = this.thread.topic.replace(/\//g, ".")
				stompApi.connect(topic, orgUid, function() {
					// 长连接成功回调
					console.log('byteDeskConnect success')
				})
			}
			// 发送附言信息
		...
		updateMessageStatus(messageObject) {
			...
			for (let i = app.messages.length - 1; i >= 0; i--) {
				...
				if (msg.uid === messageObject.content) {
					...
					app.messages[i].status = messageObject.type
					return
				}
			}
		},
		...
		gotoRatePage () {
			//
			uni.navigateTo({
				url: './rate?tid=' + this.thread.uid + '&invite=' + this.isInviteRate
			});
		},
		//更多功能(点击+弹出) 
		showMore() {
